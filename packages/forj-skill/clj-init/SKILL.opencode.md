...
**CRITICAL: Do NOT use the question tool for the project name.** Just output the question as text. The name is free-form text, not multiple choice.
...
3. Once they confirm (e.g., "done, called it myapp"):
   ...
   - Tell them: `cd myapp && opencode` then use `/clj-repl`
...
```
cd my-app && opencode
# then use /clj-repl to start the REPL
```
...
```
ClojureDart is initialized automatically during scaffolding. REPL starts automatically - watch for the ClojureDart REPL listening message in output, then connect with `nc localhost XXXXX`.

**If project has BOTH backend AND Flutter:**
```
cd my-app && opencode   # For the Clojure backend, use /clj-repl
# In another terminal:
bb flutter               # For Flutter frontend
```

**IMPORTANT:** Do NOT offer to start the REPL from the current session. The user should exit and restart OpenCode in the new project directory so that:
- Plugin detects the correct project type
